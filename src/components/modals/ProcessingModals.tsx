...
import { useUpdateProductToProcessedMutation, useDeleteProductToProcessedMutation } from "../../services/productToProcessedService";
import type { ProductToProcessed } from "../../models/productToProcessed";
import { formatNumber } from "../../utilities/formatters";
import "../css/Modal.css";
import "../css/Forms.css";

interface ProcessingEditModalProps {
  show: boolean;
  handleClose: () => void;
  item: ProductToProcessed | null;
}

function ProcessingEditModal({ show, handleClose, item }: ProcessingEditModalProps) {
  const [updateProduct, { isLoading }] = useUpdateProductToProcessedMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("İşlemdeki ürün başarıyla güncellendi!");
      handleClose();
    } catch (err: any) {
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton className="modal-header-fistik">
        <Modal.Title>
          <i className="bi bi-pencil me-2"></i>İşlemdeki Ürün Düzenle
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
...
interface ProcessingDeleteModalProps {
  show: boolean;
  handleClose: () => void;
  item: ProductToProcessed | null;
}

function ProcessingDeleteModal({ show, handleClose, item }: ProcessingDeleteModalProps) {
  const [deleteProduct, { isLoading }] = useDeleteProductToProcessedMutation();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("İşlemdeki ürün başarıyla silindi!");
      handleClose();
    } catch (err: any) {
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton className="bg-danger text-white">
        <Modal.Title>
          <i className="bi bi-exclamation-triangle me-2"></i>İşlemdeki Ürün Sil
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export { ProcessingEditModal, ProcessingDeleteModal };
