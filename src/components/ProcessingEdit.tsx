...
import { useGetProductToProcessedsQuery, useUpdateProductToProcessedMutation, useDeleteProductToProcessedMutation } from "../services/productToProcessedService";
...
function ProcessingEdit() {
  const { data, isLoading, isError } = useGetProductToProcessedsQuery();
  const [updateProduct, { isLoading: isUpdating }] = useUpdateProductToProcessedMutation();
  const [deleteProduct, { isLoading: isDeleting }] = useDeleteProductToProcessedMutation();

  const [showEditModal, setShowEditModal] = useState(false);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ProductToProcessed | null>(null);
  const [formData, setFormData] = useState({ productName: "", description: "", amount: 0 });

  const handleOpenEditModal = (item: ProductToProcessed) => {
    setSelectedItem(item);
    setFormData({ productName: item.productName, description: item.description || "", amount: item.amount });
    setShowEditModal(true);
  };

  const handleOpenDeleteModal = (item: ProductToProcessed) => {
    setSelectedItem(item);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateProduct({ ...selectedItem, productName: formData.productName, description: formData.description, amount: formData.amount }).unwrap();
    ...
    <div className="container-fluid px-4 mt-4">
      <div className="card shadow-sm">
        <div className="card-header card-header-fistik text-white d-flex justify-content-between align-items-center">
          <h5 className="mb-0"><i className="bi bi-pencil-square me-2"></i>İşlemdeki Ürünleri Düzenle</h5>
