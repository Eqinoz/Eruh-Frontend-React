...
import { useGetProductsQuery, useUpdateProductMutation, useDeleteProductMutation } from "../services/productService";
...
function ProductEdit() {
  ...
  const [formData, setFormData] = useState({ name: "", packagingType: "" });

  const handleOpenEditModal = (item: ProductModel) => {
    setSelectedItem(item);
    setFormData({ name: item.name, packagingType: item.packagingType });
    setShowEditModal(true);
  };

  const handleOpenDeleteModal = (item: ProductModel) => {
    setSelectedItem(item);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateProduct({ ...selectedItem, name: formData.name, packagingType: formData.packagingType }).unwrap();
    ...
    <div className="container-fluid px-4 mt-4">
      <div className="card shadow-sm">
        <div className="card-header card-header-fistik text-white d-flex justify-content-between align-items-center">
          <h5 className="mb-0"><i className="bi bi-pencil-square me-2"></i>Satışa Hazır Ürünleri Düzenle</h5>
          <Badge bg="light" text="dark">{data?.data.length || 0} Kayıt</Badge>
        </div>
        <div className="card-body p-0">
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="thead-fistik"><tr><th>ID</th><th>Ürün Adı</th><th>Paketleme Tipi</th><th className="text-center">İşlemler</th></tr></thead>
              <tbody>
                {data && data.data.length > 0 ? data.data.map((item) => (
                  <tr key={item.id}>
                    <td>#{item.id}</td><td>{item.name}</td><td>{item.packagingType}</td>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)} centered>
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3"><Form.Label className="fw-bold">Ürün Adı</Form.Label><Form.Control type="text" name="name" value={formData.name} onChange={handleChange} autoFocus /></Form.Group>
            <Form.Group className="mb-3"><Form.Label className="fw-bold">Paketleme Tipi</Form.Label><Form.Control type="text" name="packagingType" value={formData.packagingType} onChange={handleChange} /></Form.Group>
          </Form>
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered>
        ...
        <Modal.Body>
          <div className="text-center py-3">
            ...
            {selectedItem && <div className="alert alert-secondary mt-3"><strong>{selectedItem.name}</strong></div>}
          </div>
        </Modal.Body>
        <Modal.Footer>
