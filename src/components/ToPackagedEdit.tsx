...
import { useGetToPackagedItemsQuery, useUpdateToPackagedItemMutation, useDeleteToPackagedItemMutation } from "../services/toPackagedService";
...
function ToPackagedEdit() {
  const { data, isLoading, isError } = useGetToPackagedItemsQuery();
  const [updateItem, { isLoading: isUpdating }] = useUpdateToPackagedItemMutation();
  const [deleteItem, { isLoading: isDeleting }] = useDeleteToPackagedItemMutation();

  const [showEditModal, setShowEditModal] = useState(false);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ToPackagedItem | null>(null);
  const [formData, setFormData] = useState({ productType: "", productName: "" });

  const handleOpenEditModal = (item: ToPackagedItem) => {
    setSelectedItem(item);
    setFormData({ productType: item.productType, productName: item.productName });
    setShowEditModal(true);
  };

  const handleOpenDeleteModal = (item: ToPackagedItem) => {
    setSelectedItem(item);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateItem({ ...selectedItem, productType: formData.productType, productName: formData.productName }).unwrap();
    ...
    <div className="container-fluid px-4 mt-4">
      <div className="card shadow-sm">
        <div className="card-header card-header-fistik text-white d-flex justify-content-between align-items-center">
          <h5 className="mb-0"><i className="bi bi-pencil-square me-2"></i>Paketlenecek Ürünleri Düzenle</h5>
          <Badge bg="light" text="dark">{data?.data.length || 0} Kayıt</Badge>
        </div>
        <div className="card-body p-0">
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="thead-fistik"><tr><th>ID</th><th>Tür (Kod)</th><th>Ürün Adı</th><th className="text-center">İşlemler</th></tr></thead>
              <tbody>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)} centered>
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3"><Form.Label className="fw-bold">Tür (Kod)</Form.Label><Form.Control type="text" name="productType" value={formData.productType} onChange={handleChange} /></Form.Group>
